...
- **The LSTM needs explicit masking, not just correctly-filled padding.** An earlier version of this notebook fed the padded sequence into the `Embedding` layer without telling it which positions were padding. Since the average real sequence (66 calls) is well under half of the fixed 152-length input, most samples end with a long run of identical padding *after* the real content - and because an LSTM reads left-to-right, its final classification state ended up computed right after that constant tail, wiping out whatever it had learned from the real calls. The result was a complete training failure: loss and accuracy stayed exactly flat at chance level for 30 straight epochs. The fix, now in the notebook, is to remap padding to index `0` and use Keras's `mask_zero=True` on the `Embedding` layer, so the LSTM skips padding timesteps entirely.
...
2. **Deep Learning (LSTM):** the ordered, padded sequence of 152 API call IDs (remapped so padding = `0` and real calls = `1`-`308`), fed through an `Embedding` layer with `mask_zero=True` into an `LSTM`, which can pick up on call-order patterns the bag-of-calls representation discards entirely. The masking is not optional here - see the note below and the notebook's Discussion section.
...
- **LSTM accuracy/loss are completely flat across every epoch (e.g. stuck at ~50% and ~0.693):** this is the exact masking failure described above. Check that the `Embedding` layer still has `mask_zero=True`, and that `X_seq` was built with the padding-to-`0` remap (`np.where(sequence_array == PAD_VALUE, 0, sequence_array + 1)`) in the Feature Extraction cell - if either was removed or reverted, the LSTM will fail to learn again.
